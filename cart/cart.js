...
function renderCart() {
  if (cartContentDiv) {
    ...
    clearBtn.className = 'btn__clear-cart';
    clearBtn.setAttribute('aria-label', 'Clear cart');
    clearBtn.innerHTML = `
          Clear Cart
    ...
    `;
    cartContentDiv.append(clearBtn);
  }
...
function cartSummary() {
  ...
  if (cartSummarySection) {
    cartSummarySection.innerHTML = `
      <div class="summary__item subtotal">
        <span>Subtotal</span>
        <span>$${subtotal}</span>
      </div>
      <div class="summary__item delivery">
        <span>Delivery cost</span>
        <span>$${delivery}</span>
      </div>
      <div class="summary__item tax">
        <span>Tax</span>
        <span>$${taxes}</span>
      </div>
      ...
      <div class="summary__item summary__item--total total">
        <span>Total</span>
        <span>$${total}</span>
      </div>
    `;
